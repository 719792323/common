...
本地缓存位于应用内部，其最大的优点是应用存在于同一个进程内部，请求本地缓存的速度非常快，不存在额外的网络开销。本地缓存有：高效、低依赖、轻量、简单、成本低等特点，但是对分布式架构缺乏支持且受部署机器内存容量影响明显。
...
答案： 那肯定是不行的！因为这样可能会造成 **数据库（db）和缓存（Cache）数据不一致**的问题。不过该方案本来就有一致性问题（**请求 1 先读数据 A，请求 2 随后写数据 A，并且数据 A 在请求 1 请求之前不在缓存中的话，也有可能产生数据不一致性的问题**），只不过改成先删除后写导致不一致的概率大很多
